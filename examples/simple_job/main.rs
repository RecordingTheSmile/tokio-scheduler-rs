...
use tokio_scheduler_rs::{JobScheduler, MemoryJobStorage, DefaultJobExecutor};
...
async fn main(){
    ...
    let job_executor = DefaultJobExecutor::new(job_storage.to_owned(), Some(10), None);
    ...
    scheduler.add_job(ExampleJob::JOB_NAME, "*/5 * * * * * *", &None).await.unwrap();
